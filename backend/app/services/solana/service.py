...
class SolanaService:
    ...
    async def transfer_usdc(
        ...
        from_ata = _get_ata(from_pk, USDC_MINT_DEVNET)
        to_ata = _get_ata(to_pk, USDC_MINT_DEVNET)
        ...
        if to_ata_info.value is None:
            instructions.append(_make_create_ata_ix(from_pk, to_ata, to_pk, USDC_MINT_DEVNET))

        amount_raw = int(amount_usdc * 10 ** DECIMALS)
        instructions.append(
            _make_transfer_checked_ix(from_ata, USDC_MINT_DEVNET, to_ata, from_pk, amount_raw, DECIMALS)
        )
