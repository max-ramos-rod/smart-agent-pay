...
class ExecutionService:
    ...
    async def create_awaiting_swap(
        self,
        db: AsyncSession,
        strategy_id: int,
        wallet_id: int,
        ...
        execution = self.repository.model(
            ...
            drop_percent=strategy.drop_percent,
            amount_sol=strategy.amount_sol,
            token=strategy.token_in or "SOL",
            amount_usdc=strategy.amount_usdc,
            status=ExecutionStatus.AWAITING_SIGNATURE,
            external_id=external_id,
            serialized_tx=serialized_tx,
            explanation=f"Swap {strategy.token_in} → {strategy.token_out} (~{out_amount_label})",
        )
